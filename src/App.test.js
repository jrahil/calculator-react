...
let title, totalCalculations, input1, input2, selectedOperator, addButton, subtractButton, multiplyButton, divideButton, resetButton, isResultExist;
...
beforeEach(() => {
	...
	totalCalculations = getByTestId('total-operations');
...
test('initial UI is rendered as expected', () => {
	expect(totalCalculations).toHaveTextContent('Total operations performed: 0');
...
test('total operations count is maintained', () => {
	...
	fireEvent.click(divideButton);
	expect(getByTestIdFunction('result')).toHaveTextContent('Result: 1');
	expect(totalCalculations).toHaveTextContent('Total operations performed: 8');
});
...
test('Reset button works', () => {
	...
	expect(totalCalculations).toHaveTextContent('Total operations performed: 1');
	expect(input1.value).toBeFalsy();
	expect(input2.value).toBeFalsy();
	expect(selectedOperator).toHaveTextContent("+");
